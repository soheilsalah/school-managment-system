...
$(document).ready(function(){
    ...
    $.ajax({
        url : "{{ route('instructor.ajax.library.book.book-category.preview') }}",
        type : "POST",
        data : {
            "_token" : "{{ csrf_token() }}",
    ...
    $.ajax({
        url : "{{ route('instructor.ajax.library.book.price-options.preview') }}",
        type : "POST",
        data : {
            "_token" : "{{ csrf_token() }}",
    ...
    $("#create-new-book").on('submit', function(e){
        ...
        if ($.inArray($("#pdf").val().split('.').pop().toLowerCase(), pdfFileExtension) == -1) {
            ...
            $.ajax({
                ...
                url : "{{ route('instructor.ajax.library.book.create') }}",
                type : "POST",
